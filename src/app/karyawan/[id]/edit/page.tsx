...
const TambahKaryawanPage = () => {
  ...
  const AmbilDataKaryawan = async () => {
    ...
    const { nama, jabatan, gaji, kota_asal, tanggal_masuk } = res.data;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await axios.put(`/api/karyawan/${id}/edit`, {
        formData,
      });
      console.log(res);
      setLoading(false);
    ...
    <div className={`flex-1 relative ${showConfirm && "gray"}`}>
      ...
      <AnimatePresence>
        {showConfirm && (
          ...
          >
            <Confirm handleConfirm={handleConfirm} title="Data berhasil dirubah"/>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
